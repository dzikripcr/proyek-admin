...
function openWhatsApp() {
    // Mengarahkan ke FAQ WhatsApp yang diminta
    window.open(
...
// Menambahkan scroll event listener untuk memastikan logo tetap terlihat
window.addEventListener("scroll", function () {
    // Tidak perlu mengubah posisi karena sudah menggunakan position: fixed
    // Fungsi ini hanya untuk demonstrasi bahwa logo tetap di tempat saat scroll
    console.log("Scrolling... Logo WhatsApp tetap di posisinya");
});

document.addEventListener("DOMContentLoaded", function () {
    // Konfirmasi delete dengan membawa pesan success
    ...
    deleteForms.forEach((form) => {
        form.addEventListener("submit", function (e) {
            ...
            // Ambil pesan success dari atribut data
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Tampilkan pesan success setelah menghapus
                    if (successMessage) {
                        Swal.fire({
                            title: "Sukses!",
                            text: successMessage,
    ...
    // Auto close alert bootstrap setelah 5 detik
...
// Script untuk menangani sidebar dan footer
document.addEventListener("DOMContentLoaded", function () {
    // Cek apakah ada sidebar di halaman
    const sidebar = document.querySelector(
        '.sidebar, [class*="sidebar"], [class*="side-nav"]'
    );

    if (sidebar) {
        // Tambahkan class has-sidebar ke body
        document.body.classList.add("has-sidebar");

        // Dapatkan lebar sidebar
        const sidebarWidth = sidebar.offsetWidth;

        // Sesuaikan margin footer
        const footerWrapper = document.querySelector(".footer-wrapper");
        if (footerWrapper && window.innerWidth >= 768) {
            footerWrapper.style.marginLeft = sidebarWidth + "px";
            footerWrapper.style.width = `calc(100% - ${sidebarWidth}px)`;
        }
    }
    ...
    window.addEventListener("resize", function () {
        ...
        if (sidebar && footerWrapper) {
            if (window.innerWidth >= 768) {
                const sidebarWidth = sidebar.offsetWidth;
                footerWrapper.style.marginLeft = sidebarWidth + "px";
                footerWrapper.style.width = `calc(100% - ${sidebarWidth}px)`;
            } else {
                footerWrapper.style.marginLeft = "0";
